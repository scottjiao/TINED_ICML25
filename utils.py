...
def get_training_config(config_path, model_name, dataset):
    if model_name == "MLP_from_sequence_of_layers":
        fetch_model_name = "MLP"
    else:
        fetch_model_name = model_name
    ...
    if fetch_model_name not in full_config[dataset]:
        model_specific_config = None
    else:
        model_specific_config = full_config[dataset][fetch_model_name]
...
def get_evaluator(dataset,):
    def evaluator(out, labels):
        pred = out.argmax(1) 
        #return pred.eq(labels).float().mean().item() # accuracy
        # macro-f1
        l=labels.cpu().numpy()
        p=pred.cpu().numpy()
        macro_f1 = f1_score( l, p, average='macro')
        micro_f1 = f1_score( l, p, average='micro')
        precision = precision_score( l, p, average='macro')
        recall = recall_score( l, p, average='macro')
        acc = accuracy_score( l, p)
        score = {'macro_f1': macro_f1, 'micro_f1': micro_f1, 'precision': precision, 'recall': recall, 'acc': acc}
        return score
