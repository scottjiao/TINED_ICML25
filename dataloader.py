...
def load_data(dataset, dataset_path, args,**kwargs):
...
def load_learned_MLP_layers(out_t_dir,device):
    posixpath=out_t_dir.joinpath("learned_MLP_part.pth")
    return torch.load(posixpath,map_location=device)

def load_learned_graph_aggregation(out_t_dir,device):
    posixpath=out_t_dir.joinpath("learned_GA_matrix_for_each_layer.pth")
    layers=torch.load(posixpath,map_location=device)
    matrices=[]
    for layer in layers:
        matrices.append(layer[0].weight)
        matrices.append(layer[0].bias)
    return matrices

def load_features_before_agg_list(out_t_dir):
    raise NotImplementedError
def load_features_after_agg_list(out_t_dir):
    raise NotImplementedError
